...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation =
      createZoneSchema.safeParse(body);
    ...
    const zone =
      await prisma.deliveryZone.create({
        data: {
          ...
          minimumOrder:
            validation.data.minimumOrder,
          fee: validation.data.fee,
          estimatedMins:
            validation.data.estimatedMins,
          coverageArea:
            validation.data.coverageArea,
        },
      });
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          'An error occurred while creating the zone',
      },
      { status: 500 }
    );
  } finally {

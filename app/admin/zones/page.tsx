...
export default function ZonesPage() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === 'deliveryFee' ? parseInt(value) || 0 : value,
    }));
  };
  ...
  const handleAddZone = () => {
    if (formData.name && formData.area) {
      ...
      setFormData({ name: '', area: '', deliveryFee: 0, deliveryTime: '' });
      setShowForm(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold text-[#1A1A2E]">Delivery Zones</h2>
        <button
          onClick={() => {
            setShowForm(true);
            setEditingId(null);
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
          <h3 className="text-xl font-bold text-[#1A1A2E]">
            {editingId ? 'Edit Zone' : 'New Delivery Zone'}
          </h3>
        ...
        {zones.map((zone) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <h4 className="text-lg font-bold text-[#1A1A2E]">{zone.name}</h4>
